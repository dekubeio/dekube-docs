...
- cnpg follows CloudNativePG's default of `enableSuperuserAccess: false`. `<cluster>-superuser` is only published when that field is true.
...
??? abstract "TL;DR"
    - Upgrade-visible: `env` > `envFrom`; all `$` escaped in command/args; `items` mounts at `<name>_<hash>`; cnpg `enableSuperuserAccess` defaults to false; extension load failure and differing top-level collisions are fatal; `subPath` honoured with a legacy fallback; ServiceMonitor `namespaceSelector`; haproxy is the fallback rewriter (priority 1100); **fakeapi: update and reconvert**
