...
- **cnpg** (after v0.2.1): `enableSuperuserAccess` defaults to `false`, as in CloudNativePG, so `<cluster>-superuser` is only published when a Cluster sets `enableSuperuserAccess: true`. An app that references that Secret needs the flag added before you regenerate.
