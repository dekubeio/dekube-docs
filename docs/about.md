...
And the fear of Kubernetes is legitimate — if you have to *maintain* it. But nobody looked. The helmfiles are literally described as "one-click deploy." There's an `init.sh`. There's complete maintenance documentation. These aren't abstruse Helm charts with 12,000 values and an ingress controller you have to configure yourself — k3s, HAProxy, one script, done. But the word "Kubernetes" was in the sentence, and that was enough. People are lazy and they don't read. That's annoying. But to be fair — usually there's nothing to read. Every Helm chart they've ever touched had a `values.yaml` the size of a novel, docs that said "see `values.yaml`," and a contribution guide that taught `git push`. So when someone shows up and says "this one is different, there's docs, there's maintenance guides, it works in one command" — they don't even check. Why would they? Nothing in their experience suggests it's true.
