...
The first time someone asked for a Docker Compose, I ignored the request. The second time, I paused and looked at *why* people were asking.

The answer is disarmingly simple: people don't want to learn Kubernetes. Not "can't" — *don't want to*. k3s exists. It's [one script](https://gist.github.com/baptisterajaut/089d4fad018129c431b675d9ef76e9d1). It runs on a Raspberry Pi. Docker Desktop has a "Enable Kubernetes" checkbox for macOS and Windows. There is no technical barrier anymore, on any platform. But people don't care. They want to paste a compose file into their NAS and move on with their life, the same way they did for Plex, for Immich, for everything else. They will vote against Kubernetes support in your GitHub issues and tell you k8s is overkill for self-hosting — while a tool that makes k8s trivial on a single node exists and is free.

And the fear of Kubernetes is legitimate — if you have to *maintain* it. But nobody looked. The helmfiles I maintain — [La Suite](https://github.com/baptisterajaut/lasuite-platform) and [stoatchat](https://github.com/baptisterajaut/stoatchat-platform) — are literally described as "one-click deploy." There's an `init.sh`. There's complete maintenance documentation. These aren't abstruse Helm charts with 12,000 values and an ingress controller you have to configure yourself — k3s, HAProxy, one script, done. But the word "Kubernetes" was in the sentence, and that was enough. People don't read — and to be fair, they've been trained not to, because usually there's nothing worth reading. Every Helm chart they've ever touched had a `values.yaml` the size of a novel, docs that said "see `values.yaml`," and a contribution guide that taught `git push`. So when someone shows up and says "this one is different, there's docs, there's maintenance guides, it works in one command" — they don't even check. Why would they? Nothing in their experience suggests it's true.

And so here we are — building an aberration to meet them where they are, because they sure as hell won't come to us. I built a script, ported two open source platforms, and it worked. It should not have worked. But it did, and that was the real danger — because a working aberration is harder to kill than a failed one.
...
- Zero documentation on internals — the defaults are baked in, the decision logic is a black box, and if it doesn't work for your use case: tough luck
