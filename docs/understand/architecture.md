...
| ConfigMap / Secret | Resolved inline into `environment:` + generated as files for volume mounts. `env` wins over `envFrom`, the last `envFrom` source wins, and `envFrom.secretRef` includes `stringData`. Secret files hold the decoded bytes (keystores stay binary); a binary value referenced as an env var is skipped with a warning. A mount with `items` gets its own `<name>_<hash>` directory. File modes follow `items[].mode` / `defaultMode` (default `0644`), with owner read-write and group/other read always added — the files are bind-mounted as your host user, and the next run rewrites them in place. |
...
| Service (NodePort / LoadBalancer) | `ports:` mapping — host port is `nodePort` for NodePort, `port` for LoadBalancer. UDP ports get `/udp`; SCTP is published as TCP with a warning. |
...
| PVC / volumeClaimTemplates | Host-path bind mounts (auto-registered in `dekube.yaml` on first run only). `volumeClaimTemplate` claims are keyed `<vct>-<sts>`; a legacy bare `<vct>` key in an existing config still resolves, with a rename warning. `subPath` mounts `<host_path>/<subPath>`; on a named volume, or with `subPathExpr`, the volume root is mounted with a warning. |
| securityContext (runAsUser, fsGroup) | Auto-generated `fix-permissions` service (`chown -R <uid>[:<fsGroup>]`, or `chgrp -R <fsGroup>` without `runAsUser`; group-writable + setgid dirs whenever `fsGroup` is set) for bind mounts and named volumes; fixed services get `group_add: [<fsGroup>]` and wait for it to complete (via the [fix-permissions](https://github.com/dekubeio/dekube-transform-fix-permissions) transform) |
...
8. **Post-process env** — port remapping and replacements applied to the environments of services whose env wasn't built through `resolve_env` (which already applied them) — catches extension-produced services. Each value gets each rewrite exactly once: neither is idempotent (80→8080 plus 8080→9090 must not chain).
...
10. **Hostname truncation** — services with names >63 chars get explicit `hostname:` (except those sharing another container's network namespace, where Docker refuses one).
...
- **Port remapping** — K8s Service port -> container port in URLs. `http://svc` (implicit port 80) and `http://svc:80` both rewritten to `http://svc:8080` if the container listens on 8080. FQDN variants (`svc.ns.svc.cluster.local:80`) are also matched. One pass per value: remaps never chain.
- **Kubelet `$(VAR)`** — `$(VAR_NAME)` in container command/args and in literal env `value`s resolved from the container's env vars (for an env value: `envFrom` plus the env vars declared before it), as kubelet does. `$$(VAR)` is kubelet's escape and stays a literal `$(VAR)`; an unknown `$(VAR)` is left as-is.
- **Command `$` escaping** — after that, every remaining `$` in command/entrypoint is doubled for compose, so `$HOME`, `${X}` and `$$` (the shell PID) reach the container's shell untouched.
...
- **String replacements** — user-defined `replacements:` from config applied once to env vars, ConfigMap files, text Secret files, and reverse proxy upstreams (`$secret:` refs in them resolved first).
