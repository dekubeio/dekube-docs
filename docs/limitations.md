...
Readiness and liveness probes are converted to compose `healthcheck` (readiness preferred, fallback to liveness). Supported probe types: `exec` (→ `CMD`), `httpGet` (→ `wget`, then `curl`, then a raw HTTP/1.0 request over bash's `/dev/tcp`; `httpHeaders` are passed on, `host` defaults to `127.0.0.1`, and a 2xx/3xx status counts as healthy), `tcpSocket` (→ `nc -z 127.0.0.1 <port>`, falling back to bash's `/dev/tcp` — plain `sh` has no `/dev/tcp`). The generated checks need one of those tools in the image: a distroless image has none, so its healthchecks always fail — override the `healthcheck:` there. An `https` httpGet probe needs `wget` or `curl` (bash's `/dev/tcp` speaks no TLS). wget follows redirects where Kubernetes doesn't, so a 3xx is only healthy if its target answers. Timing fields (`periodSeconds`, `timeoutSeconds`, `failureThreshold`, `initialDelaySeconds`) are mapped to their compose equivalents.
