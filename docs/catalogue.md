...
Handles `rewrite-target`, `use-regex`, `backend-protocol`, CORS, `proxy-body-size`, `configuration-snippet`. `rewrite-target` is emulated when it's a plain prefix strip, capture groups included (`/api(/|$)(.*)` + `/$2`, `/api/(.*)` + `/$1`); any other target warns and passes through unrewritten. A `use-regex` path without `rewrite-target` becomes a prefix match when it's a literal prefix plus a trailing wildcard (`/api(/|$)(.*)`, `/foo/.*`); anything else (alternation, character classes) warns and falls back to its literal prefix, which is broader than the regex — extra paths under it reach the backend. Access control (`auth-url`, `auth-type`, `whitelist-source-range`, `denylist-source-range`, client-cert mTLS via `auth-tls-secret` unless `auth-tls-verify-client: "off"`) isn't emulated and warns loudly: those routes are open in compose. Catch-alls (`spec.defaultBackend`, rules without a host) are skipped with a warning. If your helmfile uses nginx annotations, install this or watch your Caddy routes silently ignore everything that made your app work.
